...
class OSS {
     ...
     */
    public function publicUpload($savefile, $filePath, $options = [])
    {
        $oss = new OSS();
        $oss->ossClient->setBucket($this->BucketName);
        return $oss->ossClient->uploadFile($savefile, $filePath, $options);
    }
     ...
     * @return boolean 上传是否成功
     */
    public function privateUpload($bucketName, $ossKey, $filePath, $options = [])
    {
        $oss = new OSS(true);
        $oss->ossClient->setBucket($this->BucketName);
     ...
     * @return boolean 上传是否成功
     */
    public function publicUploadContent($ossKey, $content, $options = [])
    {
        $oss = new OSS();
        $oss->ossClient->setBucket($this->BucketName);
